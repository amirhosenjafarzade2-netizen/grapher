import matplotlib.pyplot as plt
import numpy as np
from adjustText import adjust_text
import matplotlib.lines as mlines

DEFAULT_COLORS = [
    '#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf',
    '#aec7e8', '#ffbb78', '#98df8a', '#ff9896', '#c5b0d5', '#c49c94', '#f7b6d2', '#c7c7c7', '#dbdb8d', '#9edae5',
    '#393b79', '#ad494a', '#8c6d31', '#d6616b', '#b5cf6b', '#7b4173', '#ce6dbd', '#637939', '#6b6ecf', '#d4b9da',
    '#843c39', '#de9ed6', '#7b4f4b', '#a55194', '#ce1256'
]

def get_y_for_scaling(entry, x_min, x_max, num_points):
    """Calculate y values for auto-scaling without plotting"""
    try:
        coeffs = np.array(entry['coefficients'], dtype=float)
        degree = len(coeffs) - 1
        if degree > 10:
            coeffs = coeffs[:11]
        p1_full = np.linspace(x_min, x_max, num_points)
        y_vals = np.polyval(coeffs, p1_full)
        valid = np.isfinite(y_vals)
        return y_vals[valid]
    except Exception:
        return np.array([])

def find_intersections(coeffs_map, x_min, x_max, y_min, y_max, auto_scale_y):
    """Find intersection points between polynomial pairs"""
    intersections = []
    for name1 in coeffs_map:
        for name2 in coeffs_map:
            if name1 >= name2:  # Avoid duplicate pairs
                continue
            coeffs1 = coeffs_map[name1]
            coeffs2 = coeffs_map[name2]
            
            if coeffs1 is None or coeffs2 is None:
                continue
                
            try:
                diff_coeffs = np.polysub(coeffs1, coeffs2)
                if len(diff_coeffs) == 0:
                    continue
                roots = np.roots(diff_coeffs)
                real_roots = roots[np.isreal(roots)].real
                
                for r in real_roots:
                    if x_min <= r <= x_max:
                        y = np.polyval(coeffs1, r)
                        if auto_scale_y or (y_min <= y <= y_max):
                            intersections.append((name1, name2, r, y))
            except Exception as e:
                print(f"Intersection calculation failed for {name1} & {name2}: {e}")
    
    return intersections

def plot_graphs(data_ref, use_colorful, num_colors, bg_color, legend_loc, custom_legends, show_grid, 
                grid_major_x, grid_minor_x, grid_major_y, grid_minor_y, x_min, x_max, y_min, y_max, 
                x_pos, y_pos, x_major_int, x_minor_int, y_major_int, y_minor_int, 
                title, x_label, y_label, plot_grouping, auto_scale_y, stop_y_exit, stop_x_exit, debug=False,
                invert_y_axis=False, figsize=(10, 6), dpi=300, intersect_mode=False, plot_deriv=False, show_integral=False):
    """
    Plot polynomial curves with advanced features including intersection detection and analytics.
    """
    figs = []
    colors = DEFAULT_COLORS[:min(num_colors, len(DEFAULT_COLORS))] if use_colorful else ['black']
    skipped_curves = []
    
    # Input validation
    if x_min > x_max:
        raise ValueError(f"x_min ({x_min}) must be <= x_max ({x_max})")
    if not data_ref:
        return figs, ["No data provided for plotting"]
    
    # Map user-friendly legend_loc to Matplotlib loc
    loc_map = {
        "Upper Right": "upper right",
        "Upper Left": "upper left",
        "Lower Right": "lower right",
        "Lower Left": "lower left",
        "Center Left": "center left",
        "Center Right": "center right",
        "Upper Center": "upper center",
        "Lower Center": "lower center",
        "Best": "best"
    }
    matplotlib_loc = loc_map.get(legend_loc, "best")

    # Parse custom legends
    custom_label_map = {}
    custom_color_map = {}
    if custom_legends:
        for line in custom_legends.split('\n'):
            line = line.strip()
            if not line:
                continue
            if ':' in line:
                try:
                    name, color = line.split(':', 1)
                    name = name.strip()
                    custom_label_map[name] = name
                    custom_color_map[name] = color.strip() if use_colorful else 'black'
                except ValueError:
                    skipped_curves.append(f"Invalid custom legend format: {line}")
            else:
                name = line.strip()
                custom_label_map[name] = name

    # Check if axes should be centered at (0,0)
    center_x = x_min < 0 < x_max
    if not auto_scale_y and y_min is not None and y_max is not None:
        center_y = y_min < 0 < y_max
    else:
        center_y = True

    # Determine number of points based on range size for better resolution
    x_range_width = x_max - x_min
    if x_range_width <= 100:
        num_points = 2000
    elif x_range_width <= 1000:
        num_points = 3000
    elif x_range_width <= 10000:
        num_points = 5000
    else:
        num_points = 8000

    def plot_single_curve(ax, entry, color, label, x_min, x_max, y_min, y_max, auto_scale_y, 
                          stop_y_exit, stop_x_exit, center_x, center_y, x_pos, y_pos, 
                          x_major_int, x_minor_int, y_major_int, y_minor_int, 
                          x_label, y_label, show_grid, grid_major_x, grid_minor_x, 
                          grid_major_y, grid_minor_y, invert_y_axis, use_colorful, title_suffix="", 
                          plot_deriv=False, show_integral=False):
        """Helper function to plot a single curve with common logic."""
        name = entry['name']
        try:
            coeffs = np.array(entry['coefficients'], dtype=float)
        except (ValueError, TypeError) as e:
            skipped_curves.append(f"Curve {name}: Invalid coefficients ({str(e)})")
            return False, None
        
        # Check polynomial degree limit
        degree = len(coeffs) - 1
        if degree > 10:
            skipped_curves.append(f"Curve {name}: Degree {degree} exceeds maximum allowed (10). Truncated to degree 10.")
            coeffs = coeffs[:11]

        # Generate x values
        p1_full = np.linspace(x_min, x_max, num_points)
        
        # Evaluate polynomial with error handling
        try:
            y_vals = np.polyval(coeffs, p1_full)
        except Exception as e:
            skipped_curves.append(f"Curve {name}: Polynomial evaluation failed ({str(e)})")
            return False, None

        if np.all(np.isnan(y_vals)) or np.all(np.isinf(y_vals)):
            skipped_curves.append(f"Curve {name}: No valid polynomial output (all NaN/Inf)")
            return False, None

        # Find valid points (finite values)
        valid = np.isfinite(y_vals)
        
        # Apply x limits
        valid = valid & (p1_full >= x_min) & (p1_full <= x_max)
        
        # Apply y limits if not auto-scaling
        if not auto_scale_y and y_min is not None and y_max is not None:
            valid = valid & (y_vals >= y_min) & (y_vals <= y_max)

        p_plot = p1_full[valid]
        y_plot = y_vals[valid]

        # Handle stop_y_exit - truncate after first exit, no re-entry
        if stop_y_exit and not auto_scale_y and y_min is not None and y_max is not None and len(p_plot) > 0:
            in_range = (y_vals >= y_min) & (y_vals <= y_max) & np.isfinite(y_vals)
            # Find first transition from in to out
            transitions = np.where(np.diff(in_range.astype(int)) < 0)[0]
            if len(transitions) > 0:
                first_exit_idx = transitions[0] + 1
                if np.any(in_range[:first_exit_idx]):
                    valid[first_exit_idx:] = False
                    p_plot = p1_full[valid]
                    y_plot = y_vals[valid]
                    skipped_curves.append(f"Curve {name}: Truncated at x={p1_full[first_exit_idx]:.2f}, y={y_vals[first_exit_idx]:.2f}")

        # Handle stop_x_exit
        if stop_x_exit and len(p_plot) > 0:
            x_valid = (p1_full >= x_min) & (p1_full <= x_max)
            x_exit_idx = np.where(~x_valid)[0]
            if len(x_exit_idx) > 0:
                first_exit_idx = x_exit_idx[0]
                valid[first_exit_idx:] = False
                p_plot = p1_full[valid]
                y_plot = y_vals[valid]
                skipped_curves.append(f"Curve {name}: X-truncated at x={p1_full[first_exit_idx]:.2f}")

        if len(p_plot) < 2:
            skipped_curves.append(f"Curve {name}: Insufficient valid points ({len(p_plot)})")
            return False, None

        # Plot the curve
        try:
            ax.plot(p_plot, y_plot, color=color, linewidth=2.5, label=label if use_colorful else None)
        except Exception as e:
            skipped_curves.append(f"Curve {name}: Plotting failed ({str(e)})")
            return False, None

        # Plot derivative if enabled
        if plot_deriv:
            try:
                deriv_coeffs = np.polyder(coeffs)
                y_deriv = np.polyval(deriv_coeffs, p1_full)
                valid_deriv = np.isfinite(y_deriv) & valid
                ax.plot(p1_full[valid_deriv], y_deriv[valid_deriv], 
                       color=color, linestyle='--', linewidth=1.5, 
                       label=f"{label} deriv" if use_colorful else None)
            except Exception as e:
                skipped_curves.append(f"Curve {name}: Derivative plotting failed ({str(e)})")

        # Compute integral if enabled
        integral_value = None
        if show_integral:
            try:
                int_coeffs = np.polyint(coeffs)
                integral_value = np.polyval(int_coeffs, x_max) - np.polyval(int_coeffs, x_min)
            except Exception as e:
                skipped_curves.append(f"Curve {name}: Integral calculation failed ({str(e)})")

        # Add label for non-colorful mode
        if not use_colorful and len(p_plot) > 0:
            y_range = (y_max - y_min) if not auto_scale_y and y_min is not None and y_max is not None else (max(y_plot) - min(y_plot))
            end_x, end_y = p_plot[-1], y_plot[-1] - 0.05 * y_range if y_range > 0 else y_plot[-1]
            
            text = ax.text(end_x, end_y, label, fontsize=8, ha='left', va='center',
                          bbox=dict(boxstyle="round,pad=0.3", facecolor=bg_color, alpha=0.8))
            if hasattr(ax, 'texts') and len(ax.texts) > 1:
                adjust_text(ax.texts, ax=ax, only_move={'points': 'y', 'text': 'xy'})

        return True, integral_value

    def setup_axes(ax, x_min, x_max, y_min, y_max, auto_scale_y, invert_y_axis, center_x, center_y, x_pos, y_pos, x_label, y_label):
        """Setup axis properties."""
        ax.set_xlabel(x_label)
        ax.set_ylabel(y_label)
        ax.set_xlim(x_min, x_max)
        if not auto_scale_y and y_min is not None and y_max is not None:
            ax.set_ylim(y_min, y_max)
        if invert_y_axis:
            ax.invert_yaxis()

        # Center spines at (0,0) if ranges include negative and positive values
        if center_x and center_y:
            ax.spines['left'].set_position('zero')
            ax.spines['bottom'].set_position('zero')
            ax.spines['right'].set_color('none')
            ax.spines['top'].set_color('none')
            ax.xaxis.set_ticks_position('bottom' if x_pos.lower() == 'bottom' else 'top')
            ax.yaxis.set_ticks_position('left' if y_pos.lower() == 'left' else 'right')
            ax.xaxis.set_label_position(x_pos.lower())
            ax.yaxis.set_label_position(y_pos.lower())
        else:
            ax.xaxis.set_label_position(x_pos.lower())
            ax.xaxis.set_ticks_position(x_pos.lower())
            ax.yaxis.set_label_position(y_pos.lower())
            ax.yaxis.set_ticks_position(y_pos.lower())

    def setup_grid_and_ticks(ax, show_grid, grid_major_x, grid_minor_x, grid_major_y, grid_minor_y, 
                             x_major_int, x_minor_int, y_major_int, y_minor_int, use_colorful):
        """Setup grid and tick marks."""
        # Ticks setup
        if x_major_int > 0:
            ax.xaxis.set_major_locator(plt.MultipleLocator(x_major_int))
        if x_minor_int > 0:
            ax.xaxis.set_minor_locator(plt.MultipleLocator(x_minor_int))
        if y_major_int > 0:
            ax.yaxis.set_major_locator(plt.MultipleLocator(y_major_int))
        if y_minor_int > 0:
            ax.yaxis.set_minor_locator(plt.MultipleLocator(y_minor_int))

        # Grid setup
        if show_grid:
            grid_color = '#D3D3D3' if use_colorful else 'black'
            ax.grid(True, which='major', color=grid_color, alpha=0.5 if use_colorful else 0.3)
            if grid_minor_x > 0 and grid_minor_y > 0:
                ax.grid(True, which='minor', color=grid_color, linestyle='--', alpha=0.5 if use_colorful else 0.2)
            ax.xaxis.set_major_locator(plt.MultipleLocator(grid_major_x))
            if grid_minor_x > 0:
                ax.xaxis.set_minor_locator(plt.MultipleLocator(grid_minor_x))
            ax.yaxis.set_major_locator(plt.MultipleLocator(grid_major_y))
            if grid_minor_y > 0:
                ax.yaxis.set_minor_locator(plt.MultipleLocator(grid_minor_y))

    def setup_legend(ax, matplotlib_loc, use_colorful, bg_color, extra_handles=None, extra_labels=None):
        """Setup legend with optional extra entries."""
        bbox = (1.05, 0.5) if 'right' in matplotlib_loc else (-0.05, 0.5) if 'left' in matplotlib_loc else \
               (0.5, 1.05) if 'upper' in matplotlib_loc else (0.5, -0.05) if 'lower' in matplotlib_loc else None
        
        if extra_handles and extra_labels:
            handles, labels = ax.get_legend_handles_labels()
            handles.extend(extra_handles)
            labels.extend(extra_labels)
            ax.legend(handles, labels, loc=matplotlib_loc, bbox_to_anchor=bbox, fontsize=8, frameon=True, edgecolor='black')
        elif use_colorful and ax.get_legend_handles_labels()[0]:
            ax.legend(loc=matplotlib_loc, bbox_to_anchor=bbox, fontsize=8, frameon=True, edgecolor='black')
        elif not use_colorful and ax.texts:
            ax.legend(['Custom Labels'], loc=matplotlib_loc, bbox_to_anchor=bbox, fontsize=8, frameon=True, edgecolor='black')

    if plot_grouping == "All in One":
        fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
        fig.patch.set_facecolor(bg_color)
        ax.set_facecolor(bg_color)
        
        all_y_vals = []
        plot_data = []
        coeffs_map = {}
        integral_values = []
        successful_plots = 0

        # First pass: collect data for auto-scaling and coeffs
        for i, entry in enumerate(data_ref):
            if 'name' not in entry or 'coefficients' not in entry:
                skipped_curves.append(f"Entry {i}: Missing 'name' or 'coefficients' key")
                continue
                
            name = entry['name']
            color = custom_color_map.get(name, colors[i % len(colors)]) if use_colorful else 'black'
            label = custom_label_map.get(name, name)
            
            y_temp = get_y_for_scaling(entry, x_min, x_max, num_points)
            if len(y_temp) > 2:
                all_y_vals.extend(y_temp)
                plot_data.append((entry, color, label))
                coeffs_map[name] = np.array(entry['coefficients'], dtype=float)[:11]
                successful_plots += 1

        # Compute auto-scale y limits
        y_min_final = y_min
        y_max_final = y_max
        if auto_scale_y and all_y_vals and successful_plots > 0:
            y_min_new = min(all_y_vals)
            y_max_new = max(all_y_vals)
            y_range = y_max_new - y_min_new
            if y_range > 0:
                y_min_final = y_min_new - 0.1 * y_range
                y_max_final = y_max_new + 0.1 * y_range
            else:
                y_min_final = y_min_new - 1
                y_max_final = y_max_new + 1
            center_y = y_min_final < 0 < y_max_final
        else:
            y_min_final = y_min if y_min is not None else -1000
            y_max_final = y_max if y_max is not None else 1000
            center_y = y_min_final < 0 < y_max_final

        # Second pass: plot curves
        for entry, color, label in plot_data:
            success, int_val = plot_single_curve(ax, entry, color, label, x_min, x_max, y_min_final, y_max_final, 
                                                auto_scale_y, stop_y_exit, stop_x_exit, center_x, center_y, x_pos, y_pos, 
                                                x_major_int, x_minor_int, y_major_int, y_minor_int, x_label, y_label, 
                                                show_grid, grid_major_x, grid_minor_x, grid_major_y, grid_minor_y, 
                                                invert_y_axis, use_colorful, plot_deriv=plot_deriv, show_integral=show_integral)
            if success:
                if show_integral and int_val is not None:
                    integral_values.append((label, int_val))
                
                if not use_colorful:
                    if hasattr(ax, 'texts') and len(ax.texts) > 1:
                        adjust_text(ax.texts, ax=ax, only_move={'points': 'y', 'text': 'xy'})

        # Setup axes, grid, ticks
        setup_axes(ax, x_min, x_max, y_min_final, y_max_final, auto_scale_y, invert_y_axis, center_x, center_y, x_pos, y_pos, x_label, y_label)
        setup_grid_and_ticks(ax, show_grid, grid_major_x, grid_minor_x, grid_major_y, grid_minor_y, 
                             x_major_int, x_minor_int, y_major_int, y_minor_int, use_colorful)

        # Handle intersections if enabled
        extra_handles = []
        extra_labels = []
        if intersect_mode and len(plot_data) > 1:
            intersections = find_intersections(coeffs_map, x_min, x_max, y_min_final, y_max_final, auto_scale_y)
            intersection_marker = mlines.Line2D([], [], color='red', marker='o', linestyle='None', markersize=5)
            extra_handles.append(intersection_marker)
            
            for name1, name2, r, y in intersections:
                extra_labels.append(f'Int {name1}&{name2}: ({r:.2f}, {y:.2f})')
                ax.plot(r, y, 'ro', markersize=5)

        # Add integral values to legend
        if show_integral and integral_values:
            for label, int_val in integral_values:
                extra_handles.append(mlines.Line2D([], [], color='none'))
                extra_labels.append(f'{label} integral: {int_val:.2f}')

        # Setup final legend
        setup_legend(ax, matplotlib_loc, use_colorful, bg_color, extra_handles, extra_labels)

        ax.set_title(f"{title} ({successful_plots} curves)")
        figs.append((fig, "All Curves"))
        
    else:
        # One plot per curve
        for i, entry in enumerate(data_ref):
            if 'name' not in entry or 'coefficients' not in entry:
                skipped_curves.append(f"Entry {i}: Missing 'name' or 'coefficients' key")
                continue
                
            name = entry['name']
            color = custom_color_map.get(name, colors[0]) if use_colorful else 'black'
            label = custom_label_map.get(name, label)

            fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
            fig.patch.set_facecolor(bg_color)
            ax.set_facecolor(bg_color)
            
            # Compute curve-specific y for auto-scaling
            y_min_curve = y_min
            y_max_curve = y_max
            if auto_scale_y:
                y_temp = get_y_for_scaling(entry, x_min, x_max, num_points)
                if len(y_temp) > 0:
                    y_min_new = min(y_temp)
                    y_max_new = max(y_temp)
                    y_range = y_max_new - y_min_new
                    y_min_curve = y_min_new - 0.1 * y_range if y_range > 0 else y_min_new - 1
                    y_max_curve = y_max_new + 0.1 * y_range if y_range > 0 else y_max_new + 1
                    center_y = y_min_curve < 0 < y_max_curve
                else:
                    y_min_curve = -1000
                    y_max_curve = 1000
                    center_y = True
            else:
                center_y = y_min_curve < 0 < y_max_curve

            success, int_val = plot_single_curve(ax, entry, color, label, x_min, x_max, y_min_curve, y_max_curve, 
                                                auto_scale_y, stop_y_exit, stop_x_exit, center_x, center_y, x_pos, y_pos, 
                                                x_major_int, x_minor_int, y_major_int, y_minor_int, x_label, y_label, 
                                                show_grid, grid_major_x, grid_minor_x, grid_major_y, grid_minor_y, 
                                                invert_y_axis, use_colorful, plot_deriv=plot_deriv, show_integral=show_integral)
            
            if not success:
                plt.close(fig)
                continue

            # Setup axes, grid, ticks
            setup_axes(ax, x_min, x_max, y_min_curve, y_max_curve, auto_scale_y, invert_y_axis, center_x, center_y, x_pos, y_pos, x_label, y_label)
            setup_grid_and_ticks(ax, show_grid, grid_major_x, grid_minor_x, grid_major_y, grid_minor_y, 
                                 x_major_int, x_minor_int, y_major_int, y_minor_int, use_colorful)

            # Add integral to legend if enabled
            extra_handles = []
            extra_labels = []
            if show_integral and int_val is not None:
                extra_handles.append(mlines.Line2D([], [], color='none'))
                extra_labels.append(f'{label} integral: {int_val:.2f}')

            setup_legend(ax, matplotlib_loc, use_colorful, bg_color, extra_handles, extra_labels)

            ax.set_title(f"{title} - {name}")
            figs.append((fig, name))

    if debug:
        print(f"Debug: Processed {len(figs)} plots, skipped {len(skipped_curves)} curves:")
        for skip in skipped_curves:
            print(f"  - {skip}")

    return figs, skipped_curves
